...
def generate_pattern_task(
    ...
    try:
        ...
        self.update_state(
            state='PROGRESS',
            meta={'progress': 5, 'status': 'Validating parameters...'}
        )
        ...
        self.update_state(
            ...
            meta={
                'progress': 40,
                'status': f'Generating {bars}-bar pattern...',
                'device': device
            }
        )
        ...
        self.update_state(
            state='PROGRESS',
            meta={'progress': 65, 'status': 'Converting to MIDI notes...'}
        )
        ...
        for bar in range(bars):
            ...
            if beats_per_bar >= 3:
                notes.append({
                    'pitch': 36,
                    'velocity': 95,
                    'time': base_tick + 2 * ticks_per_beat
                })
            ...
            if beats_per_bar >= 2:
                notes.append({
                    'pitch': 38,
                    'velocity': 90,
                    'time': base_tick + ticks_per_beat
                })
            if beats_per_bar >= 4:
                notes.append({
                    'pitch': 38,
                    'velocity': 92,
                    'time': base_tick + 3 * ticks_per_beat
                })
        ...
        if humanize:
            self.update_state(
                state='PROGRESS',
                meta={'progress': 80, 'status': 'Applying style transfer...'}
            )
        ...
        self.update_state(
            state='PROGRESS',
            meta={'progress': 90, 'status': 'Exporting MIDI file...'}
        )
...
def tokenize_midi(
    ...
    try:
        self.update_state(
            state='PROGRESS',
            meta={'progress': 10, 'status': 'Loading MIDI file...'}
        )
        ...
        self.update_state(
            state='PROGRESS',
            meta={'progress': 25, 'status': 'Extracting metadata...'}
        )
        ...
        self.update_state(
            state='PROGRESS',
            meta={'progress': 50, 'status': 'Tokenizing MIDI...'}
        )
        ...
        self.update_state(
            state='PROGRESS',
            meta={'progress': 75, 'status': 'Saving tokens...'}
        )
